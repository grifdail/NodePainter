...
const UPGRADES: UpgradeFunction[] = [
    ...
    (sketch) => {
        ...
        return addMissingNodePort(["Draw/Image"], sketch);
    },
    (sketch) => {
        Object.values(sketch.nodes).forEach((node) => {
            if (node.type === "Math/Vector/VectorFromAngle") {
                node.type = node.selectedType === "vector3" ? "Math/Vector/SphericalCoordinate" : "Math/Vector/PolarCoordinate";
            }
        });
        return sketch;
    },
    (sketch) => {
        Object.values(sketch.nodes).forEach((node) => {
            if (node.type === "Random/OnSphere" && node.selectedType === "vector2") {
...
export function upgradeSaveData(sketch: SketchSave) {
    const templateVersion = sketch.version || 0;
    ...
    for (let i = templateVersion; i < UPGRADES.length; i++) {
...
function addMissingNodePort(list: string[], sketch: SketchSave): SketchSave {
    Object.values(sketch.nodes).forEach((node) => {
        if (list.includes(node.type)) {
            const def = NodeLibrary[node.type];
...
function addMissingSettings(list: string[], sketch: SketchSave): SketchSave {
    Object.values(sketch.nodes).forEach((node) => {
        if (!list.includes(node.type)) {
            return;
        }
        const def = NodeLibrary[node.type];
        const defaultsValues = createSettingObjectForSettingDefinition(def.settings);
        def.settings.forEach((setting) => {
            if (!node.settings[setting.id]) {
                node.settings[setting.id] = defaultsValues[setting.id]
            }
        });
    });
    return sketch;
