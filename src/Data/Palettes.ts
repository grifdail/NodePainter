import { ColorPalette, Gradient, GradientCollection, PaletteCollection, createColor } from "../Types/vectorDataType";
...
let DefaultPalettes: PaletteCollection = {
...
fetch("https://grifdail.fr/color/palettes_rgbf.json").then(response => {
    if (response.ok) {
    ...
    DefaultPalettes = Object.fromEntries(Object.entries(value).map(([key, palette]) => [key, (palette as [number, number, number][]).map(c => [...c, 1])]))
}).catch(err => console.error("Could not fetch the default palette remotely"));
