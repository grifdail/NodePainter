...
export function NodeSelectionModal({ close }: { close: () => void }) {
    const a = extractFilteredEdgeTypeFromEditedEdge();
        ...
        () =>
            nodeLibrary.filter((item) => {
                if (searchTerm.tags.length > 0) {
                    for (const target of searchTerm.tags) {
                ...
                if (searchTerm.input) {
                    ...
                    } else if (!portListIncludeType(item.dataInputs, searchTerm.input)) {
                        return false;
                    }
                }
                if (searchTerm.output) {
                    ...
                    } else if (!portListIncludeType(item.dataOutputs, searchTerm.output)) {
                        return false;
                    }
                }
    ...
    const onClickNode = useCallback(
        (node: NodeDefinition) => {
            const view = useViewbox.getState();
            const modalInfo = useNodeSelectionModal.getState().targetPosition;
            const target = modalInfo || ([view.x + window.innerWidth * 0.5 * view.scale, view.y + window.innerHeight * 0.5 * view.scale] as const);
            let targetTypeChange: PortType | null = searchTerm.input && node.hasInput ? node.hasInput(searchTerm.input, node) : null;
        ...
        [addNode, close, nodeFav, searchTerm.input, searchTerm.output]
    );
...
function extractFilteredEdgeTypeFromEditedEdge() {
    ...
    if (selectedNode.hasSelection) {
        ...
        if (selectedNode.location === "output") {
            return `input:${selectedNode.type} `;
        }
    }
    return a;
}
...
function sortWithPriority<T>(...comparator: ((a: T, b: T) => number)[]) {
    return (a: T, b: T) => {
        for (const element of comparator) {
