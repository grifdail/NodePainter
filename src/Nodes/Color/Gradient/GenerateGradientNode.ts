...
export const GenerateGradientNode: NodeDefinition = {
    ...
    getData: (portId, node, context) => {
        var list: Gradient = [];
        for (let i = 0; i < 10; i++) {
            if (node.dataInputs[`color-${i}`] && node.dataInputs[`pos-${i}`]) {
                list.push({
                    pos: context.getInputValueNumber(node, `pos-${i}`),
                    color: context.getInputValueColor(node, `color-${i}`),
                });
            }
        }
