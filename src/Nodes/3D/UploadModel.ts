...
export const UploadModel: NodeDefinition = {
  ...
  dataInputs: [Port.vector3("position"), Port.vector3("dimension"), Port.vector3("rotation")],
  ...
  getData(portId, node, context) {
    if (node.settings.model != null) {
      var key = `${node.id}-model-cache`;
      var keyLoading = `${node.id}-model-loading`;
