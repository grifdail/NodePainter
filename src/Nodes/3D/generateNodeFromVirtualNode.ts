...
export function generateNodeFromVirtualNode(materialVirtualNodeType: SimpleNodeVirtualNodeType<any, any>): NodeDefinition {
    ...
    return {
        ...
        getData: (portId, node, context) => {
            const params = inputs.map((port) => context.getInputValue(node, port.id, port.type));
            return materialVirtualNodeType.generate(context.getCallId(node, materialVirtualNodeType.getHash(...params)), [], ...params);
        },
    };
}
...
export function generateMaterialNodeFromVirtualNode(materialVirtualNodeType: SimpleNodeVirtualNodeType<any, any>): NodeDefinition {
    ...
    return {
        ...
        getData: (portId, node, context) => {
            ...
            const id = context.getCallId(node, materialVirtualNodeType.getHash(...params));
