...
export const MathExpressionNode: NodeDefinition = {
    id: "Math/Advanced/Expression",
    ...
    getData: (portId, node, context) => {
        const text = node.settings.expression as string;
        var inputs = Object.fromEntries(
            Object.values(node.dataInputs).map((port) => {
                return [`$${port.id}`, context.getInputValueNumber(node, port.id)];
            })
        );
