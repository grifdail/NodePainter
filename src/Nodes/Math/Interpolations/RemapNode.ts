...
export const RemapNode: NodeDefinition = {
    ...
    getData: (portId, node, context) => {
        ...
        const inmin = context.getInputValueNumber(node, "inmin");
        const inmax = context.getInputValueNumber(node, "inmax");
        const outmin = context.getInputValueVector(node, "outmin");
        const outmax = context.getInputValueVector(node, "outmax");
        const clamp = context.getInputValueBoolean(node, "clamp");
        const dt = (t - inmin) / (inmax - inmin);
        const result = vectorLerp(outmin, outmax, clamp ? Math.max(Math.min(dt, 1), 0) : dt);

        return enforceCorrectVectorTypeForNode(node, result);
    },
