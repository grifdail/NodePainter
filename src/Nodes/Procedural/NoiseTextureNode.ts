...
export const NoiseTextureNode: NodeDefinition = {
    ...
    description: "Generate a static noise texture",
    dataInputs: [],
    dataOutputs: [
...
function generateNoiseTexture(dim: Vector2, offset: Vector2, scale: Vector2, octave: number, influence: number, seed: number) {
    if (seed < 0) {
        seed = Math.random();
    }
