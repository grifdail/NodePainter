...
const keyShaderH = "BLUR_EFFECT_SHADER_H";
const keyShaderV = "BLUR_EFFECT_SHADER_V";
const keyFirstPass = "BLUR_EFFECT_FIRST_PASS";
...
export const BlurEffect: NodeDefinition = {
  ...
  getData(portId, node, context) {
    ...
    let shaderH = context.blackboard[keyShaderH] as p5.Shader;
    if (!shaderH) {
      shaderH = context.p5.createShader(vertexShader, fragShader);
      context.blackboard[keyShaderH] = shaderH;
    }
    let shaderV = context.blackboard[keyShaderV] as p5.Shader;
    if (!shaderV) {
      shaderV = context.p5.createShader(vertexShader, fragShader);
      context.blackboard[keyShaderV] = shaderV;
    }
    const keyCache = `${node.id}-image-cache`;
    let firstPass = getPassBuffer(context, keyFirstPass, image);
